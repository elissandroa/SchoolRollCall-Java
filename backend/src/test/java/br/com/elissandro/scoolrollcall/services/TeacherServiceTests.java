...
public class TeacherServiceTests {
	...
	public void setUp() throws Exception {
		...
		page = new PageImpl<>(List.of(Teacher));
		...
		when(repository.findById(existingId)).thenReturn(Optional.of(new Teacher()));
		when(repository.findById(nonExistingId)).thenReturn(Optional.empty());
		
		when(repository.findAll((Pageable)ArgumentMatchers.any())).thenReturn(page);
		
		when(repository.save(ArgumentMatchers.any())).thenReturn(Teacher);
		
		when(repository.getReferenceById(existingId)).thenReturn(Teacher);
	...
	public void findByIdShouldReturnTeacherDTOWhenIdExists() {
		TeacherDTO result = service.findById(existingId);
	...
	public void findAllPagedShouldReturnPage() {
		...
		Page<TeacherDTO> result = service.findAllPaged(pageable);
		Assertions.assertNotNull(result);
		Mockito.verify(repository, Mockito.times(1)).findAll(PageRequest.of(0, 10));
	}
	
	@Test
	public void insertShouldReturnTeacherDTO() {
		TeacherDTO result = service.insert(TeacherDTO);
	...
	@Test
	public void updateShouldReturnTeacherDTOWhenIdExists() {
		TeacherDTO result = service.update(existingId, TeacherDTO);
